...
float WC_ICM42688::getTemperature(void)
{
  float value;
  if(FIFOMode){
    value = (_temp/2.07) + 25;
...
float WC_ICM42688::getAccelDataX(void)
{
  float value;
  if(FIFOMode){
    value = _accelX;
...
float WC_ICM42688::getAccelDataY(void)
{
  float value;
  if(FIFOMode){
    value = _accelY;
...
float WC_ICM42688::getAccelDataZ(void)
{
  float value;
  if(FIFOMode){
    value = _accelZ;
...
float WC_ICM42688::getGyroDataX(void)
{
  float value;
  if(FIFOMode){
    value = _gyroX;
...
float WC_ICM42688::getGyroDataY(void)
{
  float value;
  if(FIFOMode){
    value = _gyroY;
...
float WC_ICM42688::getGyroDataZ(void)
{
  float value;
  if(FIFOMode){
    value = _gyroZ;
...
void WC_ICM42688:: tapDetectionInit(uint8_t accelMode)
{
  uint8_t bank = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
  if(accelMode == 0){
...
void WC_ICM42688::getTapInformation()
{
  uint8_t data;
...
uint8_t WC_ICM42688:: numberOfTap()
{
  return _tapNum;
}
uint8_t WC_ICM42688:: axisOfTap()
{
  return _tapAxis;
}
void WC_ICM42688:: wakeOnMotionInit()
{
  uint8_t bank = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
...
void WC_ICM42688:: setWOMTh(uint8_t axis,uint8_t threshold)
{
  uint8_t bank = 4;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
...
void WC_ICM42688:: setWOMInterrupt(uint8_t axis)
{
  uint8_t bank = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
  if(_INTPin == 1){
...
void WC_ICM42688::enableSMDInterrupt(uint8_t mode)
{
  uint8_t bank = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
  uint8_t INT = 1<<3 ;
...
uint8_t WC_ICM42688::readInterruptStatus(uint8_t reg)
{
  uint8_t bank = 0;
  uint8_t status = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
...
bool WC_ICM42688::setODRAndFSR(uint8_t who,uint8_t ODR,uint8_t FSR)
{
  bool ret = true;
  uint8_t bank = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
  if(who == GYRO){
    ...
    if(ODR > ODR_500HZ || FSR > FSR_3){
      ...
      switch(FSR){
        case FSR_0:
          _accelRange = 0.488f;// 4000/8194 = 0.488f
          break;
        case FSR_1:
          _accelRange = 0.244f;// 2000/8194 = 0.244f
          break;
        case FSR_2:
          _accelRange = 0.122f;// 1000/8194 = 0.122f
          break;
        case FSR_3:
          _accelRange = 0.061f;// 500/8194 = 0.061f
          break;
      }
    }
  } 
...
void WC_ICM42688::setFIFODataMode()
{
  uint8_t bank = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
...
void WC_ICM42688::startFIFOMode()
{
  uint8_t bank = 0;
  FIFOMode = true;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
...
void WC_ICM42688::setINTMode(uint8_t INTPin,uint8_t INTmode,uint8_t INTPolarity,uint8_t INTDriveCircuit)
{
  uint8_t bank = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
  if(INTPin == 1){
...
void WC_ICM42688::startTempMeasure()
{
  PWRMgmt0.tempDis = 0;
  uint8_t bank = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
...
void WC_ICM42688::startGyroMeasure(uint8_t mode)
{
...
void WC_ICM42688::startAccelMeasure(uint8_t mode)
{
...
void WC_ICM42688:: setGyroNotchFilterFHz(double freq,uint8_t axis)
{
  uint8_t bank = 1;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
...
void WC_ICM42688::setGyroNFbandwidth(uint8_t bw)
{
  uint8_t bank = 1;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
...
void WC_ICM42688::setGyroNotchFilter(bool mode)
{
  if(mode){
...
void WC_ICM42688::setAAFBandwidth(uint8_t who,uint8_t BWIndex)
{
  uint8_t bank = 0;
...
void WC_ICM42688::setAAF(uint8_t who,bool mode)
{
  uint8_t bank = 0;
  if(who == GYRO){
    if(mode){
...
bool WC_ICM42688::setUIFilter(uint8_t who,uint8_t filterOrder ,uint8_t UIFilterIndex)
{
  bool ret = true;
  uint8_t bank = 0;
  writeReg(ICM42688_REG_BANK_SEL,&bank,1);
...
WC_ICM42688_I2C::WC_ICM42688_I2C(uint8_t i2cAddr,TwoWire *pWire)
{
  _deviceAddr = i2cAddr;
  _pWire = pWire;
}

int WC_ICM42688_I2C::begin(void)
{
  _pWire->begin();
  return WC_ICM42688::begin();
}

void WC_ICM42688_I2C::writeReg(uint8_t reg, void* pBuf, size_t size)
{
  if(pBuf == NULL){
...
uint8_t WC_ICM42688_I2C::readReg(uint8_t reg, void* pBuf, size_t size)
{
  if(pBuf == NULL){
    DBG("pBuf ERROR!! : null pointer");
  }
...
WC_ICM42688_SPI::WC_ICM42688_SPI(uint8_t csPin,SPIClass *pSpi)
{
  _pSpi = pSpi;
  _csPin = csPin;
}

int WC_ICM42688_SPI::begin(void)
{
  _pSpi->begin();
...
void WC_ICM42688_SPI::writeReg(uint8_t reg, void* pBuf, size_t size)
{
  if(pBuf == NULL){
     DBG("pBuf ERROR!! : null pointer");
  }
...
uint8_t WC_ICM42688_SPI::readReg(uint8_t reg, void* pBuf, size_t size)
{
  if(pBuf == NULL){
	  DBG("pBuf ERROR!! : null pointer");
  }
